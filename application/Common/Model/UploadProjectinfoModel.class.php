...
class UploadProjectinfoModel extends CommonModel
{
    ...
    {
        $data = M('upload_projectinfo as a')
    ...
    public function getProject($where, $page)
    {
        $data = M('upload_projectinfo a')
            ->join(C('DB_PREFIX') . 'project b on a.project_id = b.id')
            ->where($where)
            ->field('a.id,a.file_name,a.adminid,a.project_id,a.type,a.file_url,a.mfile_name,a.mfile_url,a.message,a.status,a.create_time,b.principal_name user_name')
            ->order("a.create_time DESC")
            ->limit($page->firstRow, $page->listRows)
            ->select();
