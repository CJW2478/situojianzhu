...
class HomebaseController extends AppframeController {
		...
		if($ucenter_syn){
			...
			if(empty($session_user)){
				if(!empty($_COOKIE['thinkcmf_auth'])  && $_COOKIE['thinkcmf_auth']!="logout"){
					...
					$where['mobile']=$auth_username;
					$user=$users_model->where($where)->find();
					if(!empty($user)){
						$is_login=true;
						if($user['type']==1)
						{
							$user =M('account')->find($user['id']);
						}else
						{
	...
	protected function  check_user(){
	    ...
	    $user_status=M('commonusers')->where(array("id"=>sp_get_current_userid()))->getField("status");
		if($user_status==2){
			echo '<div style="font-size:40px;text-align:center;padding:10px;margin-top:80px;overflow:hidden;">你的账户已被冻结，请联系管理员！</div>';
			exit;
		}
	}
