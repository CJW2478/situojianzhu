...
class StructuralController extends AdminbaseController {
	// 平面图
	public function index(){				
		...
		if($project_id)
		{
			$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and p.id='$project_id' and s.role_name='结构专业' and p.isdel=0")->group('p.id desc')->find();
			...
			foreach ($firstpic as $key => $value) {
				...
				$countinfo = M('upload_message')->where("adminid='".session('ADMIN_ID')."' and upid='".$value['id']."' and role_name='结构专业'")->find();
				if($countinfo)
				{
					$firstpic[$key]['isdo'] =1;
		...
		{
				$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and p.isdel=0 and s.role_name='结构专业' and s.status=1")->group('p.id')->order('s.create_time desc')->find();
			}else{
				$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and p.isdel=0 and s.role_name='结构专业' and s.status=1 and s.by_distribution='".session('ADMIN_ID')."'")->group('p.id')->order('s.create_time desc')->find();
			}
			if(!empty($info))
			{
				// 上传资料信息
		...
		if(session('ADMIN_ID')==1 || !empty($finfo))
		{
			$list=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='结构专业' and p.isdel=0 and s.status=1")->group('p.id')->order('p.create_time desc')->select();
			
		}else{
			$list=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='结构专业' and p.isdel=0 and s.status=1 and s.by_distribution='".session('ADMIN_ID')."'")->group('p.id')->order('p.create_time desc')->select();
		}
		...
		if(IS_POST)
		{
			...
			redirect(U('Structural/index',array('project_id'=>$pdata['project_id'])));						
		}
		if($info['id']>0){
			// 各专业数据未读数
	...
	public function zyinfolist()
	{
		...
		$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and p.id='$project_id' and s.role_name='结构专业' and p.isdel=0 and s.status=1")->group('p.id desc')->find();
		// 建筑专业相关的项目
		$finfo=D()->field('ru.*')->table(C('DB_PREFIX')."role_user ru,".C('DB_PREFIX')."role r")->where("r.name='负责人' and r.id=ru.role_id and ru.user_id='".session('ADMIN_ID')."'")->find();
		if(session('ADMIN_ID')==1 || !empty($finfo))
		{
	...
	public function bzysglist()
	{
		...
		if($info['id']>0){
			...
			// 本专业施工图
			$count = M('uploadzysg_data')->where("project_id='".$project_id."' and rolename='结构专业'")->count();
			$page =$this->page($count,8);
			$zysgfile =M('uploadzysg_data')->where("project_id='".$project_id."' and rolename='结构专业'")->limit($page->firstRow , $page->listRows)->order('createtime desc')->select();
	...
	public function doinglist()
	{
		...
		// 进度动态
		if($info['id']>0){
			$count =M('project_doings')->where("project_id='".$project_id."' and rolename='结构专业'")->count();
			$page =$this->page($count,8);
			$files =M('project_doings')->where("project_id='".$project_id."' and rolename='结构专业'")->limit($page->firstRow,$page->listRows)->order('createtime desc')->select();
			foreach ($files as $key => $value) {			
				$files[$key]['user_name'] =M('users')->where("id='".$value['adminid']."'")->getField('user_name');						
			}
			// 各专业数据未读数
	...
	public function uploadzyfile()
	{
		if(IS_POST)
		{
			...
			if($lastid)
			{
				...
				$this->ajaxReturn(array('status'=>0,'html'=>$html,'type'=>$pdata['type'],'url'=>U('Admin/Structural/zyinfolist',array('project_id'=>$pdata['project_id']))));
			}else
			{
	...
	public function uploadzysg()
	{
		if(IS_POST)
		{
			...
			if($lastid)
			{
				...
				$this->ajaxReturn(array('status'=>0,'html'=>$html,'type'=>$pdata['type'],'url'=>U('Admin/Structural/bzysglist',array('project_id'=>$pdata['project_id']))));
			}else
			{
				$this->ajaxReturn(array('status'=>1));
			}
	...
	public function getmessage()
	{
		if(IS_POST)
		{
			...
			$this->ajaxReturn(array('status'=>0,'html'=>$content,'filename'=>$info['filename'],'urldata'=>$info['urldata'],'url'=>U('Admin/Structural/download2',array('id'=>$id))));
		}
	}
	// 获取平面图的反馈意见
	public function getpmmessage()
	{
		if(IS_POST)
		{
			...
			$content=empty($content) ? '暂无内容': $content;
			$this->ajaxReturn(array('status'=>0,'html'=>$content,'filename'=>$info['refilename'],'urldata'=>$info['reurldata'],'url'=>U('Admin/Structural/download_pm',array('id'=>$id))));
		}
	}
	public function getggmesage()
	{
		if(IS_POST)
		{
			...
			$content=empty($content) ? '暂无内容': $content;
			$this->ajaxReturn(array('status'=>0,'html'=>$content,'filename'=>$info['mfilename'],'urldata'=>$info['murldata'],'url'=>U('Admin/Structural/downloadgg',array('id'=>$id))));
		}
	}
	// 获取我的反馈
	public function getmessageinfo()
	{
		if(IS_POST)
		{
			...
			$content =htmlspecialchars_decode($info['content']);
			$content = empty($content) ? '暂无内容': $content;
			$this->ajaxReturn(array('status'=>0,'content'=>$content,'filename'=>$info['filename'],'url'=>U('Admin/Structural/download2',array('id'=>$id))));
		}
	}
	// 下载文件
	...
	public function adddoing()
	{
		if(IS_POST)
		{
			...
			if($res)
			{
				$this->ajaxReturn(array('status'=>0,'url'=>U('Admin/Structural/doinglist',array('project_id'=>$pdata['project_id']))));
			}
		}
	}
