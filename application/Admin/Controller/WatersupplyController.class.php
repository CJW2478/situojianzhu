...
class WatersupplyController extends AdminbaseController {
	// 平面图
	public function index(){				
		...
		if($project_id)
		{
			$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and p.id='$project_id' and s.role_name='给排水专业' and p.isdel=0")->group('p.id desc')->find();
			...
			foreach ($firstpic as $key => $value) {
				...
				$countinfo = M('upload_message')->where("adminid='".session('ADMIN_ID')."' and upid='".$value['id']."' and role_name='给排水专业'")->find();
				if($countinfo)
				{
					$firstpic[$key]['isdo'] =1;
		...
		else
		{
			...
			{
				$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='给排水专业' and p.isdel=0 and s.status=1")->group('p.id')->order('s.create_time desc')->find();
			}else{
				$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='给排水专业' and p.isdel=0 and s.status=1 and s.by_distribution='".session('ADMIN_ID')."'")->group('p.id')->order('s.create_time desc')->find();
			}
			if($info)
			{
				// 上传资料信息
		...
		if(session('ADMIN_ID')==1 || !empty($finfo))
		{
			$list=D()->field('p.*,s.status')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='给排水专业' and p.isdel=0 and s.status=1")->order('s.create_time desc')->group('p.id')->select();
		}else{
			$list=D()->field('p.*,s.status')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='给排水专业' and p.isdel=0 and s.status=1 and s.by_distribution='".session('ADMIN_ID')."'")->order('s.create_time desc')->group('p.id')->select();
		}
		// 各专业数据未读数
		//$zyncount =M('uploadzy_data')->where("adminid != session('ADMIN_ID') and isread=0 and project_id='".$info['id']."'")->count();
		//$this->assign('zyncount',$zyncount);
		$minfo = M('upload_message')->where("adminid='".session('ADMIN_ID')."' and upid='".$firstpic['id']."' and role_name='给排水专业'")->find();
		if(IS_POST)
		{
			$pdata =I('post.');
			$pdata['role_name'] ='给排水专业';
			...
			redirect(U('Watersupply/index',array('project_id'=>$pdata['project_id'])));						
		}
		// 各专业数据未读数
		if($info['id']>0){
			...
			$ucount=M('uploadreadlog')->where("project_id='".$info['id']."' and modelname='upload_data' and adminid='".SESSION('ADMIN_ID')."' and type=2")->getField('number');
	...
	public function zyinfolist()
	{
		...
		$info=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and p.id='$project_id' and s.role_name='给排水专业' and p.isdel=0 and s.status=1")->group('p.id desc')->find();
		// 给排水专业相关的项目
		$finfo=D()->field('ru.*')->table(C('DB_PREFIX')."role_user ru,".C('DB_PREFIX')."role r")->where("r.name='负责人' and r.id=ru.role_id and ru.user_id='".session('ADMIN_ID')."'")->find();
		if(session('ADMIN_ID')==1 || !empty($finfo))
		{
			$list=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='给排水专业' and p.isdel=0 and s.status=1")->order('s.create_time desc')->group('p.id')->select();
		}else{
			$list=D()->field('p.*')->table(C('DB_PREFIX')."solutions s,".C('DB_PREFIX')."project p")->where("p.id=s.project_id and s.role_name='给排水专业' and p.isdel=0 and s.status=1 and s.by_distribution='".session('ADMIN_ID')."'")->order('s.create_time desc')->group('p.id')->select();
		}
		// 各专业条件图
		if($info['id']>0){
	...
	public function bzysglist()
	{
		...
		if($info['id']>0){
			$count1 = M('uploadpic_data')->where("project_id='".$project_id."' and  status2=1")->count();
			$this->assign('count1',$count1);
			$count = M('uploadzysg_data')->where("project_id='".$project_id."' and rolename='给排水专业'")->count();
			$page =$this->page($count,8);
			$zysgfile =M('uploadzysg_data')->where("project_id='".$project_id."' and rolename='给排水专业'")->limit($page->firstRow , $page->listRows)->order('createtime desc')->select();
	...
	public function doinglist()
	{
		...
		// 进度动态
		if($info['id']>0){
		$count =M('project_doings')->where("project_id='".$project_id."' and rolename='给排水专业'")->count();
		$page =$this->page($count,8);
		$files =M('project_doings')->where("project_id='".$project_id."' and rolename='给排水专业'")->limit($page->firstRow,$page->listRows)->order('createtime desc')->select();
		foreach ($files as $key => $value) {			
			$files[$key]['user_name'] =M('users')->where("id='".$value['adminid']."'")->getField('user_name');						
		}
		// 各专业数据未读数
	...
	public function uploadzyfile()
	{
		if(IS_POST)
		{
			...
			if($lastid)
			{
				...
				$this->ajaxReturn(array('status'=>0,'html'=>$html,'type'=>$pdata['type'],'url'=>U('Admin/Watersupply/zyinfolist',array('project_id'=>$pdata['project_id']))));
			}else
			{
	...
	public function uploadzysg()
	{
		if(IS_POST)
		{
			...
			if($lastid)
			{
				...
				$this->ajaxReturn(array('status'=>0,'html'=>$html,'type'=>$pdata['type'],'url'=>U('Admin/Watersupply/bzysglist',array('project_id'=>$pdata['project_id']))));
			}else
			{
				$this->ajaxReturn(array('status'=>1));
			}
	...
	public function getmessage()
	{
		if(IS_POST)
		{
			...
			$this->ajaxReturn(array('status'=>0,'html'=>$content,'filename'=>$info['filename'],'urldata'=>$info['urldata'],'url'=>U('Admin/Watersupply/download2',array('id'=>$id))));
		}
	}
	// 获取平面图的反馈意见
	public function getpmmessage()
	{
		if(IS_POST)
		{
			...
			$content=empty($content) ? '暂无内容': $content;
			$this->ajaxReturn(array('status'=>0,'html'=>$content,'filename'=>$info['refilename'],'urldata'=>$info['reurldata'],'url'=>U('Admin/Watersupply/download_pm',array('id'=>$id))));
		}
	}
	public function getggmesage()
	{
		if(IS_POST)
		{
			...
			$content=empty($content) ? '暂无内容': $content;
			$this->ajaxReturn(array('status'=>0,'html'=>$content,'filename'=>$info['mfilename'],'urldata'=>$info['murldata'],'url'=>U('Admin/Watersupply/downloadgg',array('id'=>$id))));
		}
	}
	// 获取我的反馈
	public function getmessageinfo()
	{
		if(IS_POST)
		{
			...
			$content=empty($content) ? '暂无内容': $content;
			$this->ajaxReturn(array('status'=>0,'content'=>$content,'filename'=>$info['filename'],'url'=>U('Admin/Watersupply/download2',array('id'=>$id))));
		}
	}
	// 下载文件
	...
	public function adddoing()
	{
		if(IS_POST)
		{
			...
			if($res)
			{
				...
				$this->ajaxReturn(array('status'=>0,'url'=>U('Admin/Watersupply/doinglist',array('project_id'=>$pdata['project_id']))));
			}
		}
	}
