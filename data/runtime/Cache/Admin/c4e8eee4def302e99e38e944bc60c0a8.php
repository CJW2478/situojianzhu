...
<body>
	<div class="wrap js-check-wrap">
		...
		<ul class="nav nav-tabs pointUl">
			...
			<li><a href="<?php echo U('Plane/communication',array('project_id'=>$project_id));?>">沟通记录文件<?php if($sign["communication"] > 0): ?><span class="pointred">.</span><?php endif; ?></a></li>
			<li><a href="<?php echo U('Plane/design',array('project_id'=>$project_id));?>">规划、指标测算、设计方案<?php if($sign["design"] > 0): ?><span class="pointred">.</span><?php endif; ?></a></li>
			<li class="active"><a href="<?php echo U('Plane/scheme',array('project_id'=>$project_id));?>">平面规划方案</a></li>
		</ul>
		...
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
					<label class="control-label" style="margin-left: 30px;">平面规划方案：</label>
			</div>
			<div class="control-group" style="margin-bottom: 0px;">
				<label class="control-label">&nbsp;</label>
				<div class="controls" style="margin-bottom: 5px;">
					
					<a href="javascript:upload_workingplan2('文件上传','#thumb','file','','','<?php echo ($project_id); ?>',1,'Plane/scheme')">
			...
			<table class="table table-hover table-bordered" style="width: 900px;margin-left:90px;">
				<thead>
					<tr>
						...
						<th style="text-align: center;min-width: 200px;">操作</th>
					</tr>
				</thead>
					...
					<?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>						
						...
						<td style="text-align: center;">
							<?php if($val['audit_status'] == 0): if($val['status'] == 0): ?><span>已确认</span><?php endif; ?>
								<?php if($val['status'] == 1): ?><span style="color:red">未处理</span><?php endif; endif; ?>
						</td>
						<td style="text-align: center;">
							<a href="<?php echo U('Admin/Plane/downloadmessage5',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
...
<script type="text/javascript">
    ...
    function show_div(message,filename,urlid) {
        ...
        if (filename) {
			...
			var url ="<?php echo U('Plane/downloadfkfile');?>"+"&id="+urlid;
