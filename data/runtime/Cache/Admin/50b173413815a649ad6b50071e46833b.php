...
<body>
	<div class="wrap js-check-wrap">
		...
		<ul class="nav nav-tabs pointUl">
			...
			<li class="active"><a href="<?php echo U('Effect/effect_plan',array('project_id'=>$principal['id']));?>">效果图方案<?php if($sign['xgcount'] > 0 ): ?><span class="pointred">.</span><?php endif; ?></a>
		...
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
					<label class="control-label" style="margin-left: 50px;width: 200px">第一次上传效果图方案：</label>
			</div>
			<div class="control-group" style="margin-bottom: 0px;">
				<label class="control-label">&nbsp;</label>
				<div class="controls" style="margin-bottom: 5px;">
					<a href="javascript:upload_effect('文件上传','#thumb','file','','','<?php echo ($project_id); ?>',1,'Effect/effect_plan')">
						<span class="btn" style="margin-left: -88px;background:#1abc9c">上传至乙方负责人</span>
					</a>
					<span style="margin-left: 20px;color:#ccc">注：首次上传需要乙方负责人确认，请上传效果图方案时一并上传一份相关介绍视频</span>
				</div>
			</div>
			<table class="table table-hover table-bordered" style="width: 900px;margin-left:90px;">
				<thead>
				...
				<tbody>
				<?php if(is_array($first_files)): $i = 0; $__LIST__ = $first_files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i; if($val["is_first"] == 1): ?><tr>
							...
							<td style="text-align: center;">
								<a href="<?php echo U('Effect/downloadeffect',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
							</td>
						</tr><?php endif; endforeach; endif; else: echo "" ;endif; ?>
				<?php if(count($first_files) == 0): ?><tr><td colspan="5" style="text-align: center;">暂无资料信息</td>
					</tr><?php endif; ?>
				</tbody>
			</table>
			<div class="control-group" style="margin-bottom: 0px;">
				<label class="control-label" style="margin-left: 20px;">效果图方案：</label>
			</div>
			<div class="control-group" style="margin-bottom: 0px;">
				<label class="control-label">&nbsp;</label>
				<div class="controls" style="margin-bottom: 5px;">
					<?php if($status == 1): ?><a href="javascript:upload_effect('文件上传','#thumb','file','','','<?php echo ($project_id); ?>',0,'Effect/effect_plan')">
							<span class="btn" style="margin-left: -88px;background:#1abc9c">上传至甲方负责人</span>
						</a>
						<?php else: ?>
						<span class="btn" style="cursor:default;margin-left: -88px;background:grey">上传至甲方负责人</span><?php endif; ?>
					<span style="margin-left: 20px;color:#ccc">注：首次上传的效果图方案经乙方负责人确认后将在此表中显示</span>
				</div>
			</div>
			<table class="table table-hover table-bordered" style="width: 900px;margin-left:90px;">
				<thead>
					<tr>
						...
						<th style="text-align: center;min-width: 120px;">文件名</th>
						<th style="text-align: center;min-width: 80px;">上传人姓名</th>
						<th style="text-align: center;min-width: 80px;">甲方负责人审核状态</th>
						<th style="text-align: center;min-width: 80px;">乙方负责人确认状态</th>
						<th style="text-align: center;min-width: 180px;">操作</th>
					</tr>
				</thead>
				<tbody id="messagelist">
					<?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>						
						...
						<td style="text-align: center;"><?php echo ($val["file_name"]); ?></td>
						<td style="text-align: center;"><?php echo ($val["user_name"]); ?></td>
						<td style="text-align: center;"><?php if($val['audit_status'] == 0): ?><span>审核通过</span><?php endif; if($val['audit_status'] == 2): ?><span>审核不通过</span><?php endif; if($val['audit_status'] == 1): ?><span style="color:red">未处理</span><?php endif; ?></td>
						<td style="text-align: center;">
							<?php if($val['audit_status'] == 0): if($val['status'] == 0): ?><span>已确认</span><?php endif; ?>
									<?php if($val['status'] == 1): ?><span style="color:red">未处理</span><?php endif; ?>
								<?php else: ?>
									<?php if($val['status'] == 0): ?><span>已确认</span><?php endif; endif; ?>
						</td>
						<td style="text-align: center;">
							<a href="<?php echo U('Effect/downloadeffect',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
...
<script type="text/javascript">
    ...
    function show_div(message,filename,urlid) {
        ...
        if (filename) {
			...
			var url ="<?php echo U('Effect/downloadeffect2');?>"+"&id="+urlid;
