...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
			<li class="active"><a href="javascript:;">园林景观及总图专业</a></li>
		</ul>
		<form class="well form-search" method="post" action="<?php echo U('Garden/index');?>">	
			项目： 
		...
		<ul class="nav nav-tabs pointUl">
			<li class="active"><a href="<?php echo U('Garden/index',array('project_id'=>$info['id']));?>">平面图</a></li>
			<li><a href="<?php echo U('Garden/zyinfolist',array('project_id'=>$info['id']));?>">各专业条件图<?php if($zyncount > 0): ?><span class="pointred">.</span><?php endif; ?></a></li>
			<li><a href="<?php echo U('Garden/bzysglist',array('project_id'=>$info['id']));?>">本专业施工图</a></li>
			<li><a href="<?php echo U('Garden/nbmessagelist',array('project_id'=>$info['id']));?>">施工图审查意见（内审、外审）<?php if($nbncount > 0): ?><span class="pointred">.</span><?php endif; ?></a></li>
			<li><a href="<?php echo U('Garden/doinglist',array('project_id'=>$info['id']));?>">进度动态</a></li>
			<li><a href="<?php echo U('Garden/allzylist',array('project_id'=>$info['id']));?>">各专业人员基本信息</a></li>
		</ul>
		...
		<fieldset>
				...
				<tbody>
					<?php if(is_array($firstpic)): $i = 0; $__LIST__ = $firstpic;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i; if($val['filename'] != ''): ?><tr>						
						...
						<td style="text-align: center;">
							<a href="<?php echo U('Garden/download',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
			...
			<form class="form-horizontal" name="tagforms3" method="post" enctype="multipart/form-data" action="<?php echo U('Garden/index');?>">
			<fieldset>
	...
	{
		console.log(val);
		location.href = '<?php echo U("Garden/index");?>'+'&project_id='+val;
	}
    ...
    function show_div2(projectid,ids,mid) {     
        ...
        $.ajax({
        	url:"<?php echo U('Admin/Garden/getmessageinfo');?>",
