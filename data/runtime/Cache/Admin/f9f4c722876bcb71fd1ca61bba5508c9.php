...
<body>
	<div class="wrap js-check-wrap">
		...
		<ul class="nav nav-tabs pointUl">
			...
			<li class="active"><a href="<?php echo U('Plane/design',array('project_id'=>$principal['id']));?>">规划、指标测算、设计方案</a></li>
		...
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
				...
				<div class="controls" style="margin-bottom: 5px;">
					<a href="javascript:upload_workingplan2('文件上传','#thumb','file','','','<?php echo ($project_id); ?>',4,'Plane/design')">
						<span class="btn" style="margin-left: -88px;background:#1abc9c">上传至乙方负责人确认</span>
					</a>
					<span style="margin-left: 20px;color:#ccc;">注：仅需上传一次，乙方负责人确认后甲方可下载</span>
				</div>
			</div>
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				<thead>
				...
				<tbody id="messagelist">
					<?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>						
						...
						<td style="text-align: center;"><?php if($val['status'] == 0): ?><span>已确认</span><?php endif; if($val['status'] == 1): ?><span style="color:red">未处理</span><?php endif; ?></td>
						<td style="text-align: center;">						
							<a href="<?php echo U('Plane/downloadgg',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
