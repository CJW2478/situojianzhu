...
<div class="wrap js-check-wrap">
	...
	<ul class="nav nav-tabs pointUl">
		...
		<li class="active"><a href="<?php echo U('Principal/plane',array('project_id'=>$project['id']));?>">平面图方案阶段方案确认</a></li>
	...
	<form class="form-horizontal" id="tagforms" method="post" enctype="multipart/form-data">
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
				<label class="control-label" style="margin-left: 80px;width: 220px">规划、指标测算、设计方案确认：</label>
			</div>
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				<thead>
				<tr>
				...
				<?php if(is_array($scheme_data)): $i = 0; $__LIST__ = $scheme_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
						...
						<td style="text-align: center;">
							...
							<?php if($val['status']==1): ?><a onclick="change_status('<?php echo ($val["id"]); ?>','<?php echo ($project_id); ?>',5)" class="btn" style="background:#1abc9c">确认</a>
								<?php else: ?>
				...
				<?php if(count($scheme_data) == 0): ?><tr><td colspan="4" style="text-align: center;">暂无资料信息</td>
					</tr><?php endif; ?>
				</tbody>
			</table>
			...
			<div class="control-group" style="margin-bottom: 0px;">
				<label class="control-label" style="margin-left: 55px;">平面规划方案确认：</label>
			</div>
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				<thead>
				<tr>
				...
				<tbody>
				<?php if(is_array($plane_data)): $i = 0; $__LIST__ = $plane_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
						<td style="text-align: center;"><?php echo (date('Y-m-d H:i:s',$val["create_time"])); ?></td>						
						<td style="text-align: center;"><?php echo ($val["file_name"]); ?></td>
						<td style="text-align: center;"><?php echo ($val["user_name"]); ?></td>
						<?php if($val['status']==0): ?><td style="text-align: center;">已确认</td>
						...
						<td style="text-align: center;">
							<a href="<?php echo U('Principal/downloadgg',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
							<?php if($val['status']==1): ?><a onclick="change_status('<?php echo ($val["id"]); ?>','<?php echo ($project_id); ?>',9)" class="btn" style="background:#1abc9c">确认</a>
								<?php else: ?>
								<a class="btn" style="background:grey;cursor: default" href ="javascript:return false;" onclick="return false;"  >已确认</a><?php endif; ?>
						</td>
					</tr><?php endforeach; endif; else: echo "" ;endif; ?>
				<?php if(count($plane_data) == 0): ?><tr><td colspan="4" style="text-align: center;">暂无资料信息</td>
					</tr><?php endif; ?>
				</tbody>
			</table>
			<div class="pagination" style="margin-left: 90px"><?php echo ($plane_page); ?></div>
		</fieldset>
	</form>
</div>
            ...
            success:function (res) {
                if(res.status == 0){
                    ...
                    setInterval(function(){
                        location.href='<?php echo U("Principal/plane");?>'+"&project_id="+project_id;
                    },3000)
                }
                else {
