...
<div class="wrap js-check-wrap">
	...
	<ul class="nav nav-tabs pointUl">
		...
		<li><a href="<?php echo U('Principal/effect',array('project_id'=>$project['id']));?>">效果图方案阶段方案确认
		<?php if($sign["effect_first"] > 0 || $sign["effect_final"] > 0 || $sign["effect_scheme"] > 0 ): ?><span class="pointred">.</span><?php endif; ?></a></li>
		<li class="active"><a href="<?php echo U('Principal/work',array('project_id'=>$project['id']));?>">施工图方案阶段方案确认</a></li>
	</ul>
	<form class="form-horizontal" id="tagforms" method="post" enctype="multipart/form-data">
		<fieldset>
			...
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				...
				<tbody>
				<?php if(is_array($work_data)): $i = 0; $__LIST__ = $work_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
						...
						<?php if($val['status']==1): ?><td style="text-align: center;">已确认</td>
						<?php else: ?>
							<td style="text-align: center;color: red">未处理</td><?php endif; ?>
						<td style="text-align: center;">
							<a href="<?php echo ($val["urldata"]); ?>" download="<?php echo ($val["filename"]); ?>" class="btn" style="background:#1abc9c">下载</a>
							<?php if($val['status']==0): ?><a onclick="change_status('<?php echo ($val["id"]); ?>','<?php echo ($project_id); ?>')" class="btn" style="background:#1abc9c">确认</a>
								<?php else: ?>
				...
				<?php if(count($work_data) == 0): ?><tr><td colspan="4" style="text-align: center;">暂无上传资料</td>
					</tr><?php endif; ?>
				</tbody>
			</table>
			<div class="pagination" style="margin-left: 90px"><?php echo ($work_page); ?></div>
		</fieldset>
		<div class="control-group" style="margin-bottom: 30px;">
			<label class="control-label">内部审核：</label>
			<div class="controls" style="margin-top: 5px;">
				<?php if($project["status"] == 0): ?><a class="btn" onclick="audit_status('<?php echo ($project_id); ?>',1)" style="background:#1abc9c">审核通过</a>
					<?php else: ?>
					<a class="btn" style="background:grey;cursor: default" href ="javascript:return false;" onclick="return false;" >审核通过</a><?php endif; ?>
		...
		<div class="control-group" style="margin-bottom: 30px;">
			...
			<div class="controls" style="margin-top: 5px;">
				<?php if($project["status"] < 2): ?><a class="btn" onclick="audit_status('<?php echo ($project_id); ?>',2)" style="background:#1abc9c">审核通过</a><?php endif; ?>
				<?php if($project["status"] == 2): ?><a class="btn" style="background:grey;cursor: default" href ="javascript:return false;" onclick="return false;" >项目已结束</a><?php endif; ?>
			</div>
		</div>
	</form>
</div>
...
<script type="text/javascript">
    ...
    function change_status(id,project_id) {
		$.ajax({
            url:'<?php echo U("Principal/confirm_uploadpic_data");?>',
            ...
            success:function (res) {
                if(res.status == 0){
                    ...
                    location.href='<?php echo U("Principal/work");?>'+"&project_id="+project_id;                   
                }
                else {
    ...
    function audit_status(project_id,status) {
        $.ajax({
            url:'<?php echo U("Principal/audit_status");?>',
