...
<div class="wrap js-check-wrap">
	...
	<ul class="nav nav-tabs pointUl">
		...
		<li><a href="<?php echo U('Distribution/solutions',array('project_id'=>$project_id));?>">分配方案师<?php if($sign["solutions"] > 0): ?><span class="pointred">.</span><?php endif; ?></a></li>
		<li class="active"><a href="<?php echo U('Distribution/professional',array('project_id'=>$project_id));?>">分配各专业人员</a></li>
	...
	<form class="form-horizontal">
		<fieldset>
			...
			<table  class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				...
				<?php if(count($data) == 0): ?><tr><td colspan="6" style="text-align: center;">暂无专业人员信息</td>
					</tr><?php endif; ?>
				</tbody>
			</table>
...
<script type="text/javascript">
    ...
    function confirm(){
		...
		{
			$.ajax({
				url:'<?php echo U("distribution/solution_add_professional");?>',
				data:{arr:arr},
				type:"POST",
				dataType:"json",
				success:function (res) {
					...
					$('#bg').css('display','none');
					if(res.status == 0){                  
						location.href='<?php echo U("distribution/professional");?>'+"&project_id="+project_id;                    
					}
					else {
    ...
    function add_row() {		
        var tr = '<tr id="tr_'+i+'" style="margin-bottom: 20px">\n' +
			...
			'<option value="0">选择角色</option>\n' +
            '<?php if(is_array($roles)): foreach($roles as $key=>$vo): ?>\n' +			
	...
	{
		$.ajax({
			url:"<?php echo U('Distribution/getroleusers');?>",
