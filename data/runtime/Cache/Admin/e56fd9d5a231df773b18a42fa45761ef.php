...
<style>
	.row-fluid{
		display:none;position: fixed;  top: 20%;border-radius: 3px;  left: 28%; width: 44%;height: 350px; overflow:hidden; overflow-y: auto;  padding: 8px;  border: 1px solid #E8E9F7;  background-color: white;  z-index:10003;
	}
...
<body>
	<div class="wrap js-check-wrap">
		...
		<ul class="nav nav-tabs pointUl">
			...
			<li class="active"><a href="<?php echo U('Effect/communication',array('project_id'=>$principal['id']));?>">沟通记录文件</a></li>
		...
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
				...
				<div class="controls" style="margin-bottom: 5px;">
					<a href="javascript:upload_communication('文件上传','#thumb','file','','','<?php echo ($project_id); ?>',4,'Effect/communication');">
			...
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				...
				<tbody id="messagelist">
					<?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>						
						...
						<a href="<?php echo U('Effect/downloadmessage',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
...
<script type="text/javascript">
    ...
    function show_div(message,filename,urlid) {
        ...
        if (filename) {
			...
			var url ="<?php echo U('Effect/comdownloadfk');?>"+"&id="+urlid;
