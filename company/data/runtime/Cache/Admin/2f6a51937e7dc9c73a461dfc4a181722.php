...
<body>
	<div class="wrap js-check-wrap">
		...
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="<?php echo U('Stage/indexlist',array('project_id'=>$info['id']));?>">平面图确认</a></li>
			<li><a href="<?php echo U('Stage/allzylist',array('project_id'=>$info['id']));?>">各专业人员基本信息</a></li>
		</ul>
		...
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
					<label class="control-label" style="margin-left: 0px;">平面图确认</label>
			</div>
				...
				<tbody>
					<?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
						...
						<td style="text-align: center;">
							...
							<?php if($val['status2'] == 2): ?><span>审核不通过</span><?php endif; ?>
						</td>						
						<td style="text-align: center;">
						  <a href="<?php echo U('Stage/download',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>
						 <?php if($val['status2'] == 0): ?><a href="javascript:;" class="btn" onclick="show_div2('<?php echo ($val["id"]); ?>')" style="background:#1abc9c">审核</a><?php endif; ?>
						 <?php if($val['status2'] == 2): if($val['recontent'] != '' || $val['refilename'] != ''): ?><a href="javascript:;" class="btn" onclick="show_div('<?php echo ($val["id"]); ?>')" style="background:#1abc9c">反馈意见</a><?php endif; endif; ?>
						</td>
		...
		<form class="form-horizontal" name="tagforms3" method="post" enctype="multipart/form-data" action="<?php echo U('Stage/indexlist');?>">
			<fieldset>	
			<div class="row-fluid2" style="display: none">
				<div style="margin-top:70px;margin-left:40px;margin-bottom: 5px">
					...
					<table id="isfileupload" style="display: none;">
						<th style="margin-left:300px">
							<td style="text-align: center;width: 300px" id="langstr">上传确认文件：</td>
							<td style="width: 300px">
									<input type="file" name="filename" id="filenamestr" value="">
							</td>
						</th>
					</table>					
				</div>
    ...
    function show_div(id) {
        ...
        $("#filedata").show();       
    	$.ajax({
        	url:"<?php echo U('Admin/Stage/getpmmessage');?>",
        	data:{id:id},
        	type:'post',
