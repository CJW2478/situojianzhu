...
<body>
	<div class="wrap js-check-wrap">
		...
		<ul class="nav nav-tabs pointUl">
			<li class="active"><a href="<?php echo U('Stage/index',array('project_id'=>$info['id']));?>">沟通记录文件</a></li>
			<li><a href="<?php echo U('Stage/indexlist',array('project_id'=>$info['id']));?>">平面图确认<?php if($pcount > 0): ?><span class="pointred">.</span><?php endif; ?></a></li>
		...
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
				...
				<div class="controls" style="margin-bottom: 5px;">	
					<?php if($info['id'] > 0): ?><a href="javascript:upload_onemessage('文件上传','#thumb','file','','','<?php echo ($info["id"]); ?>',1,'Stage/index');">
			...
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				...
				<tbody id="messagelist">
					<?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>						
						...
						<td style="text-align: center;">						
						<a href="<?php echo U('Admin/Stage/downloadmessage',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>	
						<?php if($val['type'] == 1 && $val['status2'] == 0): ?><a href="javascript:;" class="btn" onclick="show_div2('<?php echo ($val["id"]); ?>')" style="background:#1abc9c">审核</a><?php endif; ?>					
			...
			<form class="form-horizontal" name="tagforms3" method="post" enctype="multipart/form-data" action="<?php echo U('Stage/index');?>">
			<fieldset>	
    ...
    function show_div(id) {
        ...
        $.ajax({
        	url:"<?php echo U('Admin/Stage/getggmesage');?>",
        	data:{id:id},
        	type:'post',
