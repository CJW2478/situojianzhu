...
<body>
	<div class="wrap js-check-wrap">
		...
		<ul class="nav nav-tabs">
			...
			<li><a href="<?php echo U('Casescheme/indexlist',array('project_id'=>$info['id']));?>">项目风格选选中<?php if($ecount > 0): ?><span class="pointred">.</span><?php endif; ?></a></li>
			<li class="active"><a href="<?php echo U('Casescheme/allzylist',array('project_id'=>$info['id']));?>">效果图方案确认</a></li>
		</ul>
		...
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
					<label class="control-label" style="margin-left: 30px;">效果图方案确认</label>
			</div>
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				<thead>
					<tr>
				...
				<tbody>
					<?php if(is_array($fanganlist)): $i = 0; $__LIST__ = $fanganlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>						
						...
						<td style="text-align: center;">
							<?php if($val['audit_status'] == 1): ?><span style="color:red">未处理</span><?php endif; ?>
							<?php if($val['audit_status'] == 0): ?><span>审核通过</span><?php endif; ?>
							<?php if($val['audit_status'] == 2): ?><span>审核不通过</span><?php endif; ?></td>
						<td style="text-align: center;">						
						<a href="<?php echo U('Admin/Casescheme/downloadfamessage',array('id'=>$val['id']));?>" class="btn" style="background:#1abc9c">下载</a>	
						<?php if($val['audit_status'] == 1): ?><a href="javascript:;" class="btn" onclick="show_div2('<?php echo ($val["id"]); ?>')" style="background:#1abc9c">审核</a><?php endif; ?>					
						<?php if($val['audit_status'] != 1): if($val['message'] != '' || $val['mfile_name'] != ''): ?><a href="javascript:;" onclick="show_div('<?php echo ($val["id"]); ?>');" class="btn" style="background:#1abc9c">我的反馈</a><?php endif; endif; ?>
						</td>
			...
			<form class="form-horizontal" name="tagforms3" method="post" enctype="multipart/form-data" action="<?php echo U('Casescheme/allzylist');?>">
			<fieldset>	
					...
					<table id="box">
						<tr style="margin-left:300px">
							...
							<td style="width: 300px">
								<select id="result" name="audit_status" style="width: 150px">
    ...
    function show_div(id) {
        ...
        $.ajax({
        	url:"<?php echo U('Admin/Casescheme/getfamesage');?>",
        	data:{id:id},
        	type:'post',
     ...
     $('#result').change(function () {
        ...
        if (status == 2) {
            ...
            var tr="<tr style='margin-left:300px' id='bz'><td style='text-align: right;width: 300px'>备注："+
                "</td><td style='width: 300px'><textarea style='height: 100px' id='message' maxlength='200' placeholder='不超过200字' name='message'></textarea></td></tr>";
            $('#box').append(tr);
        }else {
        	$("#langstr").html('上传确认文件：');
            $('#bz').remove();
