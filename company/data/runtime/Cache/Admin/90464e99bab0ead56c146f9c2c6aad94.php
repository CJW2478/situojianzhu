...
<div class="wrap js-check-wrap">
	...
	<ul class="nav nav-tabs pointUl">
		...
		<li class="active"><a href="<?php echo U('Caseplan/scheme',array('project_id'=>$principal['id']));?>">平面规划方案确认</a>
	</ul>
	<form class="form-horizontal" id="tagforms" method="post" enctype="multipart/form-data">
		<fieldset>
			...
			<div class="control-group" style="margin-bottom: 0px;">
				<label class="control-label" style="margin-left: 40px;">平面规划方案确认</label>
			</div>
			<table class="table table-hover table-bordered" style="width: 800px;margin-left:90px;">
				<thead>
				<tr>
				...
				<?php if(is_array($files)): $i = 0; $__LIST__ = $files;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
						...
						<td style="text-align: center;">
							<a href="<?php echo U('Admin/Caseplan/downloadfamessage',array('file_url'=>$val['file_url'],'file_name'=>$val['file_name'],'project_id'=>$val['project_id']));?>" class="btn" style="background:#1abc9c">下载</a>
							...
							<?php if($val['audit_status'] != 1): if($val['message'] != '' || $val['mfile_name'] != ''): ?><a href="javascript:;" onclick="show_div('<?php echo ($val["message"]); ?>','<?php echo ($val["mfile_name"]); ?>','<?php echo ($val["id"]); ?>');" class="btn" style="background:#1abc9c">我的反馈</a><?php endif; endif; ?>
						</td>
	...
	<form class="form-horizontal" name="tagforms3" method="post" enctype="multipart/form-data" action="<?php echo U('Caseplan/scheme');?>">
		<fieldset>
...
<script type="text/javascript">
    ...
    select.onchange = function(){
        var val = this.value;
        location.href = '<?php echo U("Caseplan/scheme");?>'+'&project_id='+val;
    };
    ...
    function show_div(message,filename,ids) {
