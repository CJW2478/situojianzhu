...
use Common\Model\UploadDataModel;
use Common\Model\UploadpicDataModel;
...
class AdminbaseController extends AppframeController {
	...
	function _initialize(){
	    ...
	    $session_admin_id=session('ADMINID');
	...
	public function initMenu() {
		$Menu = F("Cmenu");
		if (!$Menu) {
			$Menu=D("Common/Cmenu")->menu_cache();
		}
		return $Menu;
	}
     ...
     * @param $step //当前阶段
     */
    public function change_stage($project_id,$step)
    {
        $projectModle = new ProjectModel();
        $project = $projectModle->where('id='.$project_id)->find();
        if ($project['projectvalue'] < $step) {
            $projectModle->save(['id'=>$project['id'],'projectvalue'=>$step]);
        }
    }
     ...
     * @param $data
     */
    public function add_uploadreadlog($data)
    {
        $readlogModle = M('uploadreadlog');
        $log = $readlogModle->where(['adminid'=>$data['adminid'],'project_id'=>$data['project_id'],'type'=>$data['type'],'modelname'=>$data['modelname'],'ids'=>$data['ids']])->find();
        if (empty($log)) {
            $readlogModle->adminid = $data['adminid'];
            $readlogModle->modelname = $data['modelname'];
            $readlogModle->project_id = $data['project_id'];
            $readlogModle->type = $data['type'];
            $readlogModle->ids = $data['ids'];
            $readlogModle->number = $data['number'];
            $readlogModle->createtime = time();
            $readlogModle->add();
        }else{
            $readlogModle->id = $log['id'];
            $readlogModle->number = $data['number'];
            $readlogModle->save();
        }
    }
